...
//| Visualizador autônomo da wave + Kalman via GPU Engine.           |
//| Cada instância possui parâmetros próprios de GPU/Kalman.         |
//+------------------------------------------------------------------+
...
#property indicator_label2  "Kalman"
#property indicator_type2   DRAW_LINE // DRAW_LINE  // DRAW_NONE
...
// Dados de entrada/saída para a GPU
double g_frames[];
   ...
   for(int i=0; i<frame_length; ++i)
     {
      ...
      if(ArraySize(g_waveOut)   > src)
         g_bufWave[dest]   = g_waveOut[src];
      if(ArraySize(g_kalmanOut) > src)
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "GPU WaveViz Solo - Kalman");
   ...
   if(!g_engine.Initialize(InpGPUDevice, InpFFTWindow, InpHop, 1, false))
     {
      Print("[WaveViz Solo Kalman] Falha ao inicializar a GPU Engine");
      return(INIT_FAILED);
     }
