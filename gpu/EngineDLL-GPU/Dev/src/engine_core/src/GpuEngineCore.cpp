...
inline double* SanitizeOutputPtr(double* ptr, const char* label)
{
    if(!ptr)
        return nullptr;
    const std::uintptr_t addr = reinterpret_cast<std::uintptr_t>(ptr);
    if(addr == static_cast<std::uintptr_t>(-1))
    {
        DebugLog(std::string("FetchResult: ignoring invalid pointer for ") + label);
        return nullptr;
    }
    return ptr;
}
                        ...
                        ResultInfo& info)
{
    ...
    wave_out              = SanitizeOutputPtr(wave_out, "wave_out");
    preview_out           = SanitizeOutputPtr(preview_out, "preview_out");
    cycles_out            = SanitizeOutputPtr(cycles_out, "cycles_out");
    noise_out             = SanitizeOutputPtr(noise_out, "noise_out");
    phase_out             = SanitizeOutputPtr(phase_out, "phase_out");
    phase_unwrapped_out   = SanitizeOutputPtr(phase_unwrapped_out, "phase_unwrapped_out");
    amplitude_out         = SanitizeOutputPtr(amplitude_out, "amplitude_out");
    period_out            = SanitizeOutputPtr(period_out, "period_out");
    frequency_out         = SanitizeOutputPtr(frequency_out, "frequency_out");
    eta_out               = SanitizeOutputPtr(eta_out, "eta_out");
    countdown_out         = SanitizeOutputPtr(countdown_out, "countdown_out");
    recon_out             = SanitizeOutputPtr(recon_out, "recon_out");
    kalman_out            = SanitizeOutputPtr(kalman_out, "kalman_out");
    confidence_out        = SanitizeOutputPtr(confidence_out, "confidence_out");
    amp_delta_out         = SanitizeOutputPtr(amp_delta_out, "amp_delta_out");
    turn_signal_out       = SanitizeOutputPtr(turn_signal_out, "turn_signal_out");
    direction_out         = SanitizeOutputPtr(direction_out, "direction_out");
    power_out             = SanitizeOutputPtr(power_out, "power_out");
    velocity_out          = SanitizeOutputPtr(velocity_out, "velocity_out");
    phase_all_out         = SanitizeOutputPtr(phase_all_out, "phase_all_out");
    phase_unwrapped_all_out = SanitizeOutputPtr(phase_unwrapped_all_out, "phase_unwrapped_all_out");
    amplitude_all_out     = SanitizeOutputPtr(amplitude_all_out, "amplitude_all_out");
    period_all_out        = SanitizeOutputPtr(period_all_out, "period_all_out");
    frequency_all_out     = SanitizeOutputPtr(frequency_all_out, "frequency_all_out");
    eta_all_out           = SanitizeOutputPtr(eta_all_out, "eta_all_out");
    countdown_all_out     = SanitizeOutputPtr(countdown_all_out, "countdown_all_out");
    direction_all_out     = SanitizeOutputPtr(direction_all_out, "direction_all_out");
    recon_all_out         = SanitizeOutputPtr(recon_all_out, "recon_all_out");
    kalman_all_out        = SanitizeOutputPtr(kalman_all_out, "kalman_all_out");
    turn_all_out          = SanitizeOutputPtr(turn_all_out, "turn_all_out");
    confidence_all_out    = SanitizeOutputPtr(confidence_all_out, "confidence_all_out");
    amp_delta_all_out     = SanitizeOutputPtr(amp_delta_all_out, "amp_delta_all_out");
    power_all_out         = SanitizeOutputPtr(power_all_out, "power_all_out");
    velocity_all_out      = SanitizeOutputPtr(velocity_all_out, "velocity_all_out");
    plv_cycles_out        = SanitizeOutputPtr(plv_cycles_out, "plv_cycles_out");
    snr_cycles_out        = SanitizeOutputPtr(snr_cycles_out, "snr_cycles_out");
