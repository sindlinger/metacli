...
def print_friendly_overview():
    ...
    print("MT:")
    print("  mt status [--repair]         Saúde do Terminal/Listener e logs")
    print("  mt attach [--indicator|--expert|--template]  Anexa alvo (defaults do projeto)")
    print("  mt detach [--indicator|--expert]             Remove alvo (defaults do projeto)")
    print("  mt ini create                 Gera .ini de inicialização")
    print() 
    print("Projeto:")
    print("  project init|save|use|show|list; project defaults set|show")
    print() 
    print("DLL (núcleo GPU/bridge):")
    print("  dll build|build-all|link|fetch-cuda|package|ping|fft|heartbeat")
    print() 
    print("Code/Dev:")
    print("  meta|install|gen-mql|badheader … (scaffolds/compile/test em breve)")
    print() 
    print("Utilitários:")
    print("  env|detect|list|kill|start (legado)")
    print() 
    ...
    if is_wsl():
        ...
        if detach:
            ...
            try:
                ...
                # Fallback: usar 'start' do cmd.exe
        ...
        if detach:
            ...
            try:
                ...
                return subprocess.call(['cmd.exe', '/C', 'start', '', exe_win] + args)
        return subprocess.call([str(exe_path)] + args)
...
def build_ini_startup(symbol: str | None,
    ...
    lines = ['[StartUp]']
    if expert:
        lines.append(f"Expert={_ini_escape(expert)}")
    if script:
...
def collect_log_targets(data_dir: Path | None) -> list[tuple[str, Path]]:
    ...
    engine_log = base / 'MQL5' / 'Libraries' / 'logs' / 'gpu_service.log'
    targets.append(('engine', engine_log))
    tester_log = base / 'MQL5' / 'Tester' / 'logs' / time.strftime('%Y%m%d.log')
    targets.append(('tester', tester_log))
    return targets
...
def send_listener_command(data_dir: Path, payload: str) -> Path:
    target_dir = win_to_wsl(data_dir) if is_wsl() else Path(data_dir)
...
bool ApplyCommand(const string line){
   ...
   if(cmd == "ATTACH_EA" && count >= 4){
      ...
      if(!ChartSetInteger(chart, CHART_EXPERT_HANDLE, (long)NULL))
         Print("ChartSetInteger falhou: ", GetLastError());
      if(!ChartSetString(chart, CHART_EXPERT_NAME, expert))
         Print("ChartSetString falhou: ", GetLastError());
      ChartRedraw();
      PrintFormat("Expert %s anexado em %s %s", expert, sym, parts[2]);
      return(true);
   }
   if(cmd == "DETACH_EA" && count >= 3){
      ...
      long chart = ChartOpen(sym, tf);
      if(chart == 0){ Print("ChartOpen falhou: ", GetLastError()); return(false);} 
      ChartSetInteger(chart, CHART_EXPERT_HANDLE, 0);
      ChartRedraw();
      PrintFormat("Expert removido de %s %s", sym, parts[2]);
      return(true);
   }
...
def ensure_source(metaeditor: Path | None,
    ...
    target = win_to_wsl(data_dir) / 'MQL5' / rel_path
    ...
    if compile_after and metaeditor and target.suffix.lower() == '.mq5':
        ...
        if rc != 0:
            print(f"[bootstrap] Falha na compilação (rc={rc}). Veja {to_windows_path(log)}")
        elif not quiet:
...
def install_file_to_mql(metaeditor: Path | None,
    ...
    dst = win_to_wsl(data_dir) / 'MQL5' / rel_path
...
def resolve_mt_context(args):
    root = getattr(args, 'root', None) or repo_root_from_here()
...
def cmd_listener_run(args):
    # Usa defaults de símbolo/período quando omitidos
    ...
    content = build_ini_startup(
        ...
        shutdown=False)
    write_text_utf16(ini, content)
    print(f"[listener] INI salvo em {ini}")
    extra = [f"/config:{to_windows_path(ini)}"]
...
def cmd_listener_ensure(args):
    ...
    try:
        # certifique-se de que o atributo 'force' exista
...
def ensure_indicator_installed(metaeditor: Path | None, data_dir: Path, src_file: Path) -> Path:
    dst = win_to_wsl(data_dir) / 'MQL5' / 'Indicators' / src_file.name
...
def cmd_listener_status(args):
    _, _, data_dir = resolve_mt_context(args)
    send_listener_command(data_dir, 'PING')
    time.sleep(0.8)
...
def chart_indicator_attach(args):
    ...
    return boot_with_ini_and_schedule(terminal, data_dir, symbol, period, [line])
...
def chart_expert_attach(args):
    ...
    return boot_with_ini_and_schedule(terminal, data_dir, symbol, period, commands=[], portable=False, profile=None)
...
def chart_screenshot(args):
    ...
    base = win_to_wsl(data_dir) / 'MQL5' / 'Files' / 'screenshots'
    ensure_dir(base)
    if out_arg:
...
def repo_root_from_here():
    """Resolve o root do projeto onde o CLI está sendo usado.
    1) Tenta identificar a raiz a partir do CWD (execução dentro do repo).
    2) Se falhar, tenta relativo ao arquivo (útil quando rodando o mtcli fora do repo).
    """
    # 1) Preferir CWD: funciona quando o CLI está fora do repo, mas chamado dentro dele
    cwd = os.path.abspath(os.getcwd())
    cur = cwd
    for _ in range(6):
        if os.path.isfile(os.path.join(cur, 'CMakeLists.txt')) or \
           os.path.isdir(os.path.join(cur, '.git')) or \
           os.path.isdir(os.path.join(cur, 'Indicators')):
            return cur
        nxt = os.path.abspath(os.path.join(cur, '..'))
        if nxt == cur:
            break
        cur = nxt
    # 2) Relativo ao arquivo do CLI
...
def warn_wsl_path(path, context):
    ...
    if norm.startswith('/mnt/'):
        print(f"[WARN] {context} located on Windows filesystem ({norm}). If Windows build tools are also using it, clean the directory from Windows or use a WSL-only folder such as {default_build_dir(repo_root_from_here())}.")
...
def cmd_kill(_args):
    if not is_windows():
        ...
        return 1
    kill_processes(['terminal64.exe', 'terminal.exe', 'metaeditor64.exe', 'metaeditor.exe', 'alglib_worker.exe'])
    return 0
...
def main():
    ...
    p = sub.add_parser('env', help='Show environment diagnostics')
    p.add_argument('--root', help='Repo root (default: auto)')
    p.set_defaults(func=cmd_env)

    p = sub.add_parser('detect', help='Show project-linked MQL5\\Libraries or list all Terminals')
    p.add_argument('--root', help='Repo root (default: auto)')
    ...
    p = sub.add_parser('list', help='List build artifacts (Release)')
    p.add_argument('--root', help='Repo root (default: auto)')
    ...
    p = sub.add_parser('ping', help='Init/teardown bridge and show version/stats')
    p.add_argument('--root', help='Repo root (default: auto)')
    ...
    p = sub.add_parser('config', help='Show or update mtcli configuration JSON')
    p.add_argument('payload', nargs='?', help='JSON payload to merge (omit to print config)')
    p.add_argument('--root', help='Repo root (default: auto)')
    p.set_defaults(func=cmd_config)

    p = sub.add_parser('fft', help='Run a synchronous FFT via mt-bridge.dll')
    p.add_argument('--dll', help='Path to mt-bridge.dll (auto-detect if omitted)')
    ...
    p = sub.add_parser('meta', help='Compile indicator/EA via MetaEditor')
    ...
    pinstall = sub.add_parser('install', help='Instala indicadores/experts na pasta MQL5')
    ...
    lr.add_argument('--portable', action='store_true', help='Inicia o Terminal em modo Portable')
    ...
    # chart commands
    pchart = sub.add_parser('chart', help='Enfileira comandos via CommandListenerEA')
    ...
    pind = sub.add_parser('indicator', help='Operações de alto nível para indicador')
    ...
    d_build.add_argument('--root', help='Repo root (default: auto)')
    ...
    d_ba.add_argument('--root', help='Repo root (default: auto)')
    ...
    d_link.add_argument('--root', help='Repo root (default: auto)')
    ...
    d_pkg.add_argument('--root', help='Repo root (default: auto)')
    ...
    p = sub.add_parser('build', help='[LEGACY] Configure+build via CMake; use dll build')
    p.add_argument('--root', help='Repo root (default: auto)')
    ...
    pba.add_argument('--root', help='Repo root (default: auto)')
    ...
    phb.add_argument('--root', help='Repo root (default: auto)')
    ...
    ptst.add_argument('--root', help='Repo root (default: auto)')
    ptst.add_argument('--config', help='Path to tester config JSON')
    ptst.set_defaults(func=cmd_tester)

    pmatrix = sub.add_parser('matrix', help='Show matrix CI workflow snippet')
    pmatrix.add_argument('--root', help='Repo root (default: auto)')
    ...
    ppack = sub.add_parser('package', help='[LEGACY] Use dll package')
    ppack.add_argument('--root', help='Repo root (default: auto)')
    ...
    pcode = sub.add_parser('code', help='Comandos de desenvolvimento (compilação etc.)')
    ...
    pr = sub.add_parser('run', help='Inicia/garante listener (não bloqueia)')
    ...
    pa = sub.add_parser('attach', help='Anexa indicador (usa defaults do projeto quando omitidos)')
    ...
    pd = sub.add_parser('detach', help='Remove indicador do gráfico (usa defaults do projeto quando omitidos)')
    ...
    ps = sub.add_parser('status', help='PING + tail de logs')
    add_mt_args(ps)
    ...
    pshot = sub.add_parser('screenshot', help='Captura screenshot do gráfico atual do projeto (atalho para chart screenshot)')
    ...
    pmt = sub.add_parser('mt', help='Controle do MetaTrader dentro do projeto (status/chart/tester/logs)')
    ...
    def cmd_mt_status(args):
        if args.repair:
            # Usa ensure para (re)iniciar, mas retorna rápido com logs
            e = argparse.Namespace(project=getattr(args,'project',None), libs=None, terminal=None, metaeditor=None, data_dir=None, force=False, force_start=False,
                                   portable=getattr(args,'portable',False), profile=getattr(args,'profile',None))
            cmd_listener_ensure(e)
        return cmd_listener_status(args)
    ...
    pg = sub.add_parser('gen-mql', help='Generate MQL5 indicator/script from YAML/preset and optionally copy to Terminal(s)')
    pg.add_argument('--root', help='Repo root (default: auto)')
    ...
    p = sub.add_parser('link', help='Create/validate junctions from project MQL5\\Libraries (or specified paths) to build-win\\<config>')
    p.add_argument('--root', help='Repo root (default: auto)')
    ...
    ps.add_argument('--data-dir', help='Path to Data Folder root (folder that contains MQL5)')
    ...
    pdfs.add_argument('--root', help='Repo root (default: auto)')
    ...
    pdfsh.add_argument('--root', help='Repo root (default: auto)')
    ...
    def _cmd_proj_init(args):
        ...
        if idx==len(detected)+1:
            libs = _prompt('Caminho completo para MQL5\\Libraries:', '/mnt/mql5/MQL5/Libraries')
            if libs.startswith('~'):
        ...
        # Caminhos explícitos
        # Preferências: /mnt/mql5/* quando existir; caso contrário /mnt/c/mql5/*
        def _def(path1, path2):
            return path1 if os.path.isfile(path1) or os.path.isdir(path1) else path2
        term_default = _def('/mnt/mql5/terminal64.exe', '/mnt/c/mql5/terminal64.exe')
        meta_default = _def('/mnt/mql5/metaeditor64.exe', '/mnt/c/mql5/metaeditor64.exe')
        data_default = os.path.dirname(os.path.dirname(terminal['libs']))
        ...
        data_dir = _prompt(f"Data Folder (raiz contendo MQL5) [default: {data_default}]:", data_default)
        # Persistir projeto
        ...
        # Linkar MQL5\\Libraries com Release atual
        largs = argparse.Namespace(
            root=root_cfg,
            release=os.path.join(root_cfg, 'build-win', 'Release'),
            config='Release',
            terminal_id=None,
            all=False,
            libs=[terminal['libs']],
        ...
        # Instalar listener (sem compilar) e garantir execução em segundo plano — não bloqueante
        try:
            bootstrap_instance(None, Path(data_dir), force=True)
        except Exception as exc:
            print('[init] Aviso: bootstrap sem compilação falhou:', exc)
        le = argparse.Namespace(project=pid, libs=None, terminal=None, metaeditor=None, data_dir=None, force=False, force_start=False)
    ...
    pi.add_argument('--root', help='Repo root (default: auto)')
    ...
    p = sub.add_parser('fetch-cuda', help='[LEGACY] Use dll fetch-cuda')
    p.set_defaults(func=cmd_fetch_cuda)

# ---- start (interactive wizard) ----
    p = sub.add_parser('start', help='Interactive setup: pick Terminal, prepare EA, generate preset, build, link and prep agents')
